...
data<-read.delim("allSim_numbers.txt")
...
vp<-matrix(ncol=5, nrow=length(data$best_counts))
fn<-matrix(ncol=5, nrow=length(data$best_counts))
fp<-matrix(ncol=5, nrow=length(data$best_counts))
...
tempseq<-vector()
presicion_samp<-matrix(ncol = 5,nrow = 25)
recall_samp<-matrix(ncol = 5,nrow = 25)
F1_score <-matrix(ncol = 5,nrow = 25)
for (i in 1:25){
...
plot(recall_samp, presicion_samp,xlim=c(0,1),ylim=c(0,1), ylab="Presicion",xlab="Recall", pch=18,cex=1,
     col=c(rep(rgb(0,0,1,alpha=0.50),25),rep(rgb(0,1,0,alpha=0.50),25),rep(rgb(1,0.4,0,alpha=0.50),25),rep(rgb(1,0,1,alpha=0.50),25),rep(rgb(0,1,1,alpha=0.50),25)))
points( recall,presicion, col=c(rgb(0,0,1), rgb(0,1,0),rgb(1,0.4,0),rgb(1,0,1),rgb(0,1,1)), pch=8,cex=3,)
legend("bottomleft", legend = c("Best", "Unique", "RepEnrich", "Telescope", "TEtranscripts"),
  col = c(rgb(0,0,1),rgb(0,1,0),rgb(1,0.4,0),rgb(1,0,1),rgb(0,1,1)), pch=18,bty = "n",pt.cex=1.5)
dev.off()


F1_score2<-cbind(as.vector(F1_score),c(rep(rgb(0,0,1),25),rep(rgb(0,1,0),25),rep(rgb(1,0.4,0),25),rep(rgb(1,0,1),25),rep(rgb(0,1,1),25)))
...
legend("bottom",legend = c("Best", "Unique", "RepEnrich", "Telescope", "TEtranscripts"),
       col = c(rgb(0,0,1),rgb(0,1,0),rgb(1,0.4,0),rgb(1,0,1),rgb(0,1,1)),pch=15,pt.cex=1.5,bty = "n", xpd = TRUE,ncol=2, inset = c(0,-0.25),cex=1)
dev.off()
